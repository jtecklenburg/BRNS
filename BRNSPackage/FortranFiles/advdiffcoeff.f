...
c          db = db0/2.0d0*erfcc((x(j)-25.0d0)/4.0d0)
c          db = db0/2.0d0*erfcc((x(j)-10.0d0)/4.0d0)
         db=0.0
         if (x(j).gt.30.0d0) db=db0
        end if
...
c           dsol = fac_sal*dsol_0(k)*(1.d0+f_T(k)*t_celsius)! Dmol(T,S)
c           dsol = dsol/(1.0d0-dlog(por(j)*por(j)))       ! in situ Dmol
c           dsol = dsol_0(k)*0.5 ! MT 29.03.2017 assumes tortuosity of 0.5
           dsol = dsol_0(k) ! MT no correction for tortuosity or temperature
           isolid=0
