...
class ACGOrchestrator:
    ...
    @staticmethod
    def _fortran_reserved_names() -> Set[str]:
        """Return names reserved by Fortran or already used in generated ACG code."""
        reserved = {
            # Fortran keywords / type names / control words
            'accept', 'access', 'allocate', 'allocatable', 'assignment', 'assign',
            'backspace', 'block', 'byte', 'call', 'case', 'character', 'close', 'common',
            'complex', 'concurrent', 'continue', 'cycle', 'data', 'deallocate', 'dimension',
            'do', 'double', 'else', 'elseif', 'end', 'enddo', 'endif', 'endfile', 'entry',
            'equivalence', 'exit', 'external', 'format', 'function', 'goto', 'if', 'implicit',
            'include', 'in', 'inquire', 'integer', 'intent', 'interface', 'kind', 'len',
            'logical', 'module', 'namelist', 'none', 'open', 'operator', 'optional', 'parameter',
            'pause', 'pointer', 'precision', 'print', 'private', 'procedure', 'program',
            'public', 'read', 'real', 'record', 'recursive', 'result', 'return', 'rewind',
            'save', 'select', 'sequence', 'stop', 'subroutine', 'sync', 'then', 'to', 'type',
            'use', 'volatile', 'where', 'while', 'write',
            # Runtime names created by the ACG generator
            'i', 'j', 'k', 'l', 'm', 'n', 'r', 't', 'x', 'y', 'z', 'dt', 'time', 'delt',
            'sp', 'spold', 'nsolid', 'ndiss', 'ncomp', 'nreac', 'nx', 'iq', 'iw', 'idb',
            'ipor', 'igrid', 'iarea', 'ic', 'sum',
            # Names used in the legacy static BRNS Fortran sources (not generated by acg.py)
            'co', 'por', 'area', 'vd', 'vs', 'spb', 'spi', 'gm', 'ac', 'ibc', 'disp',
            'dsol_0', 'f_t', 'nssnow', 'x_pos', 'y_pos', 'z_pos', 'ii', 'jj', 'kk', 'jx',
            'nt', 'ntopt2', 'isol', 'newton', 'isplit', 'ngtzero', 'ilud',
            'nsstate', 'itssmax', 'itintmax', 'idrdc', 'istst', 'dtt', 'dttest1',
            'spguess', 'spdt1', 'spdt2', 'tstart', 'tend', 'dtold', 'ttol', 'tstep',
            'maxconc', 'optftol', 'problim', 'relsig', 'stdmin', 'perturb', 'tfact', 'etol',
            'ftol', 'ssrelax', 'alphacn', 'uc', 'q0', 'w0', 'db0', 'por0', 'area0', 't_celsius',
            'salin', 'depthmax', 'endt',
            # Additional static common-block and solver variable names found across the BRNS legacy sources
            'aa', 'bb', 'cc', 'dd', 'ee', 'fvec', 'fmin', 'frac', 'func', 'g', 'h', 'm0', 'm1',
            'm2', 'n0', 'n1', 'nmax', 'np', 'tol', 'tolx', 'tolf', 'tolmin', 'stpmx', 'isolid',
            'itransp', 'ntot', 'nopt', 'ntopt', 'seed', 'sseed', 'ranx', 'ua', 'va', 'spg',
            'di', 'rnum', 'rden', 'dtemp', 'erfcc', 'erf', 'dabs', 'dsqrt', 'dlog', 'dexp',
            'dcos', 'dsin', 'dtan', 'dmax1', 'dmin1', 'idum', 'iflag', 'check', 'iter', 'indx',
            'indxx', 'mat', 'rstate', 'newtv', 'tsconc', 'tsparam', 'tsp', 'tsp2', 'boun',
            'dr1', 'dr2', 'dr3', 'dr4', 'dr5', 'xold', 'gac', 'gb', 'mvc', 'mu', 'nu', 'niter',
            'idpar', 'spmeas', 'xmeas', 'sigmeas', 'abs', 'max', 'min', 'sqrt', 'log', 'exp',
            'sin', 'cos', 'tan', 'atan', 'asin', 'acos', 'sinh', 'cosh', 'tanh', 'ceiling',
            'floor', 'a', 'b', 'c', 'd', 'e', 'f', 'l', 'm', 'n', 'r', 's', 't', 'u', 'v', 'w',
            'x', 'y', 'z',
        }
        return {name.lower() for name in reserved}
    ...
    def _validate_references(self) -> List[ValidationIssue]:
        ...
        param_paths_by_name: Dict[str, str] = {}
        for _, pname, _, value_path in self._iter_parameter_entries(params):
            if pname in seen_param_names:
                ...
                param_paths_by_name[pname] = value_path
            seen_param_names.add(pname)

        # R-7: species/parameter collisions and reserved Fortran identifiers
        reserved_fortran_names = self._fortran_reserved_names()
        for i, name in enumerate(species_names):
            if name.lower() in reserved_fortran_names:
                issues.append(ValidationIssue(
                    'ERROR', f"species[{i}].name",
                    f"Species name '{name}' is a reserved Fortran identifier and would overwrite generated code variables.",
                    "Choose a different species name."
                ))
            if name in param_paths_by_name:
                issues.append(ValidationIssue(
                    'ERROR', f"species[{i}].name",
                    f"Species name '{name}' is not unique: it is also used as a parameter name.",
        ...
        for section_name, pname, _, value_path in self._iter_parameter_entries(params):
            ...
            if pname in known_species:
                issues.append(ValidationIssue(
                    'ERROR', value_path,
                    f"Parameter name '{pname}' is not unique: it is also used as a species name.",
                    "Use unique names across species and parameters."
                ))
